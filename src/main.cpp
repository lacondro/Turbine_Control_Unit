...
void loadSettings() {
  ...
  settings.glowOnTime = preferences.getULong("glowOnTime", 3000);
  settings.starterRampTime = preferences.getULong("starterRampTime", 2000);
  ...
  settings.ignitionCheckTime = preferences.getULong("ignitionCheckTime", 5000);
  settings.ignitionCheckTemp = preferences.getFloat("ignitionCheckTemp", 300.0);
  settings.overTemperature = preferences.getFloat("overTemperature", 750.0);
...
void saveSettings() {
  ...
  preferences.putULong("ignitionCheckTime", settings.ignitionCheckTime);
  preferences.putFloat("ignitionCheckTemp", settings.ignitionCheckTemp);
...
// --- PWM Input ISRs ---
void IRAM_ATTR isrPowerPWM() { portENTER_CRITICAL_ISR(&powerMux); if (digitalRead(POWER_PWM_PIN) == HIGH) { powerPulseStart = micros(); } else { if (powerPulseStart > 0) { latestPowerPWM = (int)(micros() - powerPulseStart); powerPulseStart = 0; if (latestPowerPWM < 800 || latestPowerPWM > 2200) latestPowerPWM = 1000; } } portEXIT_CRITICAL_ISR(&powerMux); }
void IRAM_ATTR isrThrottlePWM() { portENTER_CRITICAL_ISR(&throttleMux); if (digitalRead(THROTTLE_PWM_PIN) == HIGH) { throttlePulseStart = micros(); } else { if (throttlePulseStart > 0) { latestThrottlePWM = (int)(micros() - throttlePulseStart); throttlePulseStart = 0; if (latestThrottlePWM < 800 || latestThrottlePWM > 2200) latestThrottlePWM = 1000; } } portEXIT_CRITICAL_ISR(&throttleMux); }
void IRAM_ATTR isrPumpRPM() { portENTER_CRITICAL_ISR(&pumpRpmMux); unsigned long nowMicros = micros(); if (pumpLastPulseTimeMicros > 0) { pumpPulsePeriodMicros = nowMicros - pumpLastPulseTimeMicros; } pumpLastPulseTimeMicros = nowMicros; portEXIT_CRITICAL_ISR(&pumpRpmMux); }
...
void handleWebSocket(AsyncWebSocket *server, AsyncWebSocketClient *client, AwsEventType type, void *arg, uint8_t *data, size_t len) {
  if (type == WS_EVT_CONNECT) {
    ...
    if (info->final && info->index == 0 && info->len == len && info->opcode == WS_TEXT) {
      ...
      if (msg == "power:1") { powerSwitch = true; }
      else if (msg == "power:0") {
        powerSwitch = false;
        if (engineState == RUNNING) { setState(COOLING); }
      }
      else if (msg == "emergency") { emergencyStop = true; }
      else if (msg.startsWith("glow:")) {
          if (engineState == IDLE) { manualGlowIntent = (msg.substring(5).toInt() == 1); }
      }
      else if (msg.startsWith("valve:")) {
          if (engineState == IDLE) { manualValveIntent = (msg.substring(6).toInt() == 1); }
      }
...
void handleFSM() {
  ...
  if (emergencyStop) { setState(ERROR_STOP); }
  ...
  switch (engineState) {
    ...
    case GLOW_ON:
      ...
      if (stateElapsedTime > settings.glowOnTime) {
        starterRampStartTime = now; rampingStarter = true;
        setState(STARTER_ON);
      }
      if (powerPWM < 1300 && !powerSwitch) { setState(IDLE); }
      break;
    ...
    case STARTER_ON:
      ...
      if (rampingStarter) {
        ...
        if (now - starterRampStartTime < rampDuration) {
          starterPWM = map(now - starterRampStartTime, 0, rampDuration, 1000, settings.targetStarterPWM);
        } else {
          starterPWM = settings.targetStarterPWM; rampingStarter = false; stateStartTime = now;
        }
      } else {
    ...
    case ERROR_STOP:
      ...
      if (engineState == COOLING && temperature > settings.coolingTemperature) {
        ...
        if (!coolPulseActive && (now - lastCoolPulseTime >= coolInterval)) {
          starterPWM = settings.coolStarterPWM; lastCoolPulseTime = now; coolPulseActive = true;
        } else if (coolPulseActive && (now - lastCoolPulseTime >= coolPulseDuration)) {
          starterPWM = 1000; coolPulseActive = false; lastCoolPulseTime = now;
      ...
      if (engineState == ERROR_STOP) {
        if (!errorAcknowledgedPowerLow && (powerPWM < 1300 && !powerSwitch)) {
          errorAcknowledgedPowerLow = true; errorResetPowerHighSeen = false;
        } else if (errorAcknowledgedPowerLow && !errorResetPowerHighSeen && (powerPWM > 1800 || powerSwitch)) {
          errorResetPowerHighSeen = true;
        } else if (errorAcknowledgedPowerLow && errorResetPowerHighSeen && (powerPWM < 1300 && !powerSwitch)) {
          setState(IDLE);
          emergencyStop = false;
        }
      }
...
void setup() {
  ...
  pinMode(PUMP_RPM_PIN, INPUT); pinMode(TURBINE_RPM_PIN, INPUT);

  if (!SPIFFS.begin(true)) { Serial.println("SPIFFS Mount Failed!"); while(1) delay(1000); }
  
  Serial.print("Initializing MAX31855...");
  if (!thermocouple.begin()) Serial.println(" ERROR!"); else { Serial.println(" OK."); }
